...
const createGrumble = async (req, res) => {
  try {
    const grumbleData = {
      ...req.body,
      profilePicture:
        req.files?.profilePicture?.[0]?.filename || "grumble-placeholder.jpg",
...
const deleteGrumble = async (req, res) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ error: error.message || "Error deleting grumble member" });
  }
};
...
const updateGrumble = async (req, res) => {
  ...
  try {
    ...
    const updatedGrumble = await Grumble.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
