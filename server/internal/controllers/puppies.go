...
func GetPuppies(c *gin.Context) {
	...
	query := `
		SELECT 
			p.id, p.litter_id, p.name, p.color, p.gender, p.status, p.description,
	...
	for rows.Next() {
		...
		galleryQuery := `
			...
			FROM images i
			JOIN puppy_gallery pg ON i.id = pg.image_id
			WHERE pg.puppy_id = $1
			ORDER BY pg.display_order ASC`
		
		imgRows, _ := database.Pool.Query(c, galleryQuery, p.ID)
...
func CreatePuppy(c *gin.Context) {
	...
	var newID int
	query := `
		INSERT INTO puppies (litter_id, name, color, gender, status, description, profile_picture_id)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id`
	
	err := database.Pool.QueryRow(c, query, 
		litterID, name, color, gender, status, desc, ppID,
	).Scan(&newID)
	...
	for i := 0; i < 6; i++ {
		...
		if imgID != nil {
			_, _ = database.Pool.Exec(c, 
				"INSERT INTO puppy_gallery (puppy_id, image_id, display_order) VALUES ($1, $2, $3)", 
				newID, *imgID, i)
		}
	}
...
func UpdatePuppy(c *gin.Context) {
	...
	var currentPPID *int
	err := database.Pool.QueryRow(c, "SELECT profile_picture_id FROM puppies WHERE id=$1", id).Scan(&currentPPID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Puppy not found"})
		return
	}
	...
	query := `
		UPDATE puppies 
		SET litter_id=$1, name=$2, color=$3, gender=$4, status=$5, description=$6, profile_picture_id=$7, updated_at=NOW()
		WHERE id=$8`

	_, err = database.Pool.Exec(c, query, 
		litterID, name, color, gender, status, desc, newPPID, id,
	)
